...
모든 A/B 테스트 관련 파일들은 `src/components/ga` 디렉토리 내에 응집도 높게 구성되어 있습니다:
...
이 A/B 테스트 시스템은 좋은 응집도를 위해 모든 관련 파일이 `src/components/ga` 디렉토리에 함께 위치해 있습니다:
...
실험은 `components/ga/experiments-config.ts` 파일에서 중앙 관리됩니다:

```tsx
// components/ga/experiments-config.ts
...
```tsx
import { useABTest } from '@/components/ga/ABTestProvider';
